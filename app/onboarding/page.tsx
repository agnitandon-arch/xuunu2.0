...
export default function OnboardingPage() {
  ...
  const [widgetMode, setWidgetMode] = useState<"wearables" | "labs">(
    "wearables"
  );
  ...
  const openWidget = async (mode: "wearables" | "labs") => {
    if (!user) {
      toast({
    ...
    try {
      const response = await fetch("/api/terra/widget", {
        ...
        body: JSON.stringify({ userId: user.uid, mode }),
      });
        ...
        return (
          <div className="space-y-6">
            <ConnectDevices
              ...
              onOpenWidget={() => openWidget("wearables")}
              onSkip={() => setStep(4)}
            />
            ...
            {terraLabsEnabled && (
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <p className="text-sm text-white/60 mb-4">
                  Connect Quest, LabCorp, or upload manual results.
                </p>
                <button
                  onClick={() => openWidget("labs")}
                  className="w-full rounded-full border border-white/20 px-4 py-3 text-sm font-semibold text-white/80 transition hover:border-white/40 hover:text-white disabled:cursor-not-allowed disabled:opacity-60"
                  disabled={isWidgetLoading}
                >
                  {isWidgetLoading && widgetMode === "labs"
                    ? "Opening labs widget..."
          ...
          <div className="w-full max-w-4xl overflow-hidden rounded-2xl border border-white/10 bg-black shadow-xl">
            <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
              <p className="text-sm font-semibold text-white">
                {widgetMode === "labs" ? "Terra Labs" : "Terra Connect"}
              </p>
              <button
                type="button"
