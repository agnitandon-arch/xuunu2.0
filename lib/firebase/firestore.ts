...
export async function getActivities(
  ...
  const activities = snapshot.docs.map((docSnapshot) => ({
    id: docSnapshot.id,
    ...docSnapshot.data(),
    createdAt: normalizeTimestamp(docSnapshot.data().createdAt) || new Date().toISOString(),
  })) as Activity[];
...
export async function getHealthDataRange(
  ...
  return snapshot.docs.map((docSnapshot) => ({
    id: docSnapshot.id,
    ...docSnapshot.data(),
    updatedAt: normalizeTimestamp(docSnapshot.data().updatedAt) || new Date().toISOString(),
  })) as HealthData[];
}
...
export async function getBiosignature(
  ...
  if (docSnap.exists()) {
    return {
      ...
      ...docSnap.data(),
      calculatedAt:
        normalizeTimestamp(docSnap.data().calculatedAt) || new Date().toISOString(),
    } as Biosignature;
  }
  return null;
}
...
// Blood Work Functions
export async function saveBloodWork(
  userId: string,
  testDate: string,
  data: Partial<BloodWorkData>
): Promise<void> {
  const docId = `${userId}_${testDate}`;
  const docRef = doc(db, 'bloodWork', docId);
...
export async function getBloodWorkHistory(
  userId: string
): Promise<BloodWorkData[]> {
  const q = query(
    collection(db, 'bloodWork'),
    where('userId', '==', userId),
  ...
  return snapshot.docs.map((docSnapshot) => ({
    ...
    ...docSnapshot.data(),
    createdAt:
      normalizeTimestamp(docSnapshot.data().createdAt) || new Date().toISOString(),
  })) as BloodWorkData[];
}
